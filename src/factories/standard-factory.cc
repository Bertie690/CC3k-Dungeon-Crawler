...
class StandardFactory : public FactoryBase {
  ...
  // Used to aid in implementing FactoryBase.
  std::shared_ptr<T> create(const Position& position);

  // Generate a tuple of (ostensibly random) arguments to be used for creating an Entity.
 ...
 public:
  ...
  // Intended to be used for creating specific entities with known parameters.
  virtual std::shared_ptr<T> create(const Position& position, const Args... args) const = 0;
};

// Partial specialization for zero arguments (omitting the private methods altogether)

template <typename T>
  requires std::is_base_of_v<Entity, T>
class StandardFactory<T> : public FactoryBase {
 public:
  // Deterministically create an Entity at the given position.
...
std::shared_ptr<T> StandardFactory<T, Args...>::create(const Position& position) {
  // apply the tuple to the create method (yes it is really this verbose)
  return std::apply(
